...
import { DeleteBoardResponseDto, GetCommentListResponseDto, GetFavoriteListResponseDto, IncreaseViewCountReponseDto, PostCommentResponseDto, PutFavoriteResponseDto } from 'apis/response/board';
...
export default function BoardDetail() {
  ...
  const increaseViewCountResponse = (responseBody: IncreaseViewCountReponseDto | ResponseDto | null) => {
  ...
  const BoardDetailTop = () => {
    ...
    const getWriteDatetimeFormat = () => {
      if(!board) return '';
      const date = dayjs(board.wrtieDateTime);
  ...
  const BoardDetailBottom = () => {
    ...
    const {
      ...
      setCurrentPage, setTotalList, setCurrentSection
    ...
    const putFavoriteResponse = ( responseBody: PutFavoriteResponseDto | ResponseDto | null) => {
      ...
      // 성공적으로 좋아요를 누르면 좋아요 리스트를 요청하고 응답을 받는다.
    ...
    const postCommentResponse = (responseBody: PostCommentResponseDto | ResponseDto | null) => {
      ...
      // 기존의 작성 내용은 초기화.
      setComment('');
      // 성공적으로 댓글을 작성하면 댓글 작성 목록을 요청해 응답을 받는다.
      if(!boardNumber) return;
